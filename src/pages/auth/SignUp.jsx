        ...
        <div className='2xl:max-w-[335px] w-full max-w-[270px] mx-auto h-full flex flex-col gap-2'>
            ...
            <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)} noValidate>
                <div className="2xl:p-4 border border-input-placeholder border-[#000E2F70] flex items-center gap-x-2 rounded-xl p-3 relative mt-2">
                    <label className="2xl:text-sm absolute bg-white text-input-label -top-2 left-5 text-[11px] px-1" htmlFor="name">Name</label>
                    <input placeholder="Enter your Phone number..." className="w-full outline-none text-[12px] text-input-placeholder" type="text" id="name" name='name' {...register('name')} />
                </div>
                <p className='text-[red]'>{errors?.password?.message}</p>
                <div className="2xl:p-4 border border-input-placeholder border-[#000E2F70] flex items-center gap-x-2 rounded-xl p-3 relative mt-2">
                    <label className="2xl:text-sm absolute bg-white text-input-label -top-2 left-5 text-[11px] px-1" htmlFor="phone">Phone</label>
                    <input placeholder="Enter your name..." className="w-full outline-none text-[12px] text-input-placeholder" type="number" id="phone" name='phone' {...register('phone')} />
                </div>
                <p className='text-[red]'>{errors?.password?.message}</p>
                <div className="2xl:p-4 border border-input-placeholder border-[#000E2F70] flex items-center gap-x-2 rounded-xl p-3 relative mt-2">
